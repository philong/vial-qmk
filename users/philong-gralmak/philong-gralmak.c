...
// quote/semicolon swap. Layers are defined with the keyboard-agnostic
...
bool process_quopostrokey(uint16_t keycode, keyrecord_t *record, uint16_t toggle_keycode) {
    ...
    // When enabled, the base layer quote key (Gralmak puts ' on the old P
    // position) acts as the Quopostrokey. Only physical key presses are
    // remapped: the P+; position combo still outputs a plain '.
    if (keycode == KC_QUOT && user_config.quopostrokey && IS_KEYEVENT(record->event) && is_layer(LAYER_BASE)) {
