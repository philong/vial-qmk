...
bool dynamic_macro_play_user(int8_t direction) {
    ...
    switch (index) {
        case 0:
            SEND_STRING_DELAY(SS_TAP(X_END) SS_TAP_CODE_DELAY SS_LSFT(SS_TAP(X_HOME)) SS_TAP_CODE_DELAY DMACRO1_TEXT1 SS_TAP(X_TAB) SS_TAP_CODE_DELAY, STRING_DELAY_INTERVAL);
            SEND_STRING_DELAY(SS_TAP(X_END) SS_TAP_CODE_DELAY SS_LSFT(SS_TAP(X_HOME)) SS_TAP_CODE_DELAY DMACRO1_TEXT2 SS_TAP(X_ENTER), STRING_DELAY_INTERVAL);
            break;
        case 1:
        default:
            SEND_STRING_DELAY(SS_TAP(X_END) SS_TAP_CODE_DELAY SS_LSFT(SS_TAP(X_HOME)) SS_TAP_CODE_DELAY DMACRO2_TEXT1 SS_TAP(X_TAB) SS_TAP_CODE_DELAY, STRING_DELAY_INTERVAL);
            SEND_STRING_DELAY(SS_TAP(X_END) SS_TAP_CODE_DELAY SS_LSFT(SS_TAP(X_HOME)) SS_TAP_CODE_DELAY DMACRO2_TEXT2 SS_TAP(X_ENTER), STRING_DELAY_INTERVAL);
    }
    return true;
}
...
bool process_punctuation_mod(uint16_t keycode, keyrecord_t *record, uint16_t toggle_keycode) {
    ...
    if (!all_mods && (is_alpha(tap_keycode) || tap_keycode == U_QUOP || tap_keycode == CM_QUOT || tap_keycode == CM_SLSH || tap_keycode == CM_SCLN)) {
        last_keycode = KC_NO;
        ...
        SEND_STRING_DELAY(backspace_str, STRING_DELAY_INTERVAL);
        comma_count = 0;
        dot_count = 0;
